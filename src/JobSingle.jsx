...
const JobSingle = () => {
    ...
    const removeEmptyFields = (obj) => {
        console.log(obj, "obj is here")
        var aaa = Object.fromEntries(Object.entries(obj).filter(([_, v]) => v != null && v !== ''));
        console.log(aaa, "aaa is here")
        return aaa
            ...
            return (
                <ul>
                    {items.map((item, index) => (
                        <li key={index}>
                            {item.replace(/<link:(.*?):(.*?)>/g, '<a href="$2" target="_blank" rel="noopener noreferrer" >$1</a>')}
                        </li>
                    ))}
                </ul>
            );
